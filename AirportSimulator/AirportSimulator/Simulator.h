...
public:
	...
	void enter_data()
	{
		...
		int gates = read_int("Please enter the number of gates this airport has: ", 1, INT_MAX);

		total_time = read_int("Please enter the simulation time (hours): ", 1, INT_MAX);
		total_time *= 60;


		//set number of gates in the service queue
		service_queue->set_gates(gates);
	...
	void show_stats()
	{
		...
		std::cout << "Average wait time for planes in arrival queue: " << landing_queue->get_total_wait() / landing_queue->get_num_served() << std::endl;
		...
		std::cout << "Average wait time for planes in departure queue: " << departure_queue->get_total_wait() / departure_queue->get_num_served() << std::endl;
