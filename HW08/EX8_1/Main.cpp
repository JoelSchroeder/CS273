...
int binary_search(const vector<T> &items, int first, int last, T target) {
	...
	if(items[((last-first)/2)+first] == target){
		return ((last-first)/2)+first;
	}else if(first+1 == last || first == last ){
		return -1;
	}

	if(items[((last-first)/2)+first] < target){
		return binary_search(items, ((last-first)/2)+first, last, target);
	}else if(items[((last-first)/2)+first] > target){
		return binary_search(items, first, ((last-first)/2)+first, target);
	}
